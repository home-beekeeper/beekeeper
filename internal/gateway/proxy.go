...
func (h *gatewayHandler) handleToolCall(w http.ResponseWriter, r *http.Request, msg JSONRPCMessage, bodyBytes []byte) {
	...
	// Also extract the tool call here for use in the scanner call sites below.
	//
	// NOTE: the former package-manager nudge merge ran here (cache-backed
	// steer-to-pnpm/bun). It was removed in v1.1.0; install posture will be
	// re-introduced in a later phase. The policy/overlay/path enforcement above
	// is unchanged.
	tc := policy.ToolCall{}
	var toolName string
