...
// defaults to policyloader.LoadPolicyDir and is overridable in tests (the same
// test-seam pattern as metadataFetchFn) so the fail-closed behavior on an
// unreadable policies directory can be exercised deterministically across
// platforms (a real unreadable dir is hard to fabricate portably).
