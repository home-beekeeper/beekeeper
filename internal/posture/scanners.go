...
// scanners.go — IMPURE (reads files via injectable readFileFn, but scan logic is
// pure string parsing). This is the file-scan half of the detection reader: it
// reads bunfig.toml / pnpm-workspace.yaml from known fixed paths and delegates to
// the pure scanBunfig / scanPnpmWorkspace string parsers.
//
// Security contract:
...
//     fuzz targets that enforce the never-panic contract.
...
//     ok=false does NOT mean crash — it means "uncertain/malformed".
//
...
// (safe default). Never panics regardless of input content.
...
//     treat hardening state as "unknown" (safe defaults). Never panics.
//
// The weakness baseline is 1440 (per pnpm 11 default). The caller
// (DetectPnpmHardening) uses minimumReleaseAgeWeaknessBaseline from config.go for
// the comparison. minimumReleaseAge=0 → weakness flagged but pnpm_hardened stays
// true.
...
// (BunScannerOK=false from that file). Never panics.
...
type HardeningResult struct {
	...
	// Note: minimumReleaseAge=0 sets WeaknessLogged=true but Hardened stays true
	// (it's the user's explicit choice, not a removal of hardening).
	WeaknessLogged bool
}

// DetectPnpmHardening checks pnpm-workspace.yaml at the given path for hardening
// configuration.
//
...
// detection, never block on parse trouble).
func DetectPnpmHardening(workspacePath string) HardeningResult {
	...
	// Weakness baseline is 1440. minimumReleaseAge=0 → weakness flagged, but
	// hardened stays true.
	if hasMinAge && minAge < minimumReleaseAgeWeaknessBaseline {
		result.WeaknessLogged = true
		// hardened stays true — user's explicit choice
	}
...
// maxAge is a generous ceiling for parseInt accumulation. It bounds the running
// total so a huge minimumReleaseAge cannot silently overflow a 64-bit int and
// wrap to an arbitrary — possibly negative — value that flips WeaknessLogged. The
// field is minutes; 1<<31 (~4084 years) is far beyond any legitimate value, so
// anything past it is treated as parse trouble → ok=false → safe default.
const maxAge = 1 << 31
