...
// TestVerifyPeerUIDLinux asserts SO_PEERCRED-based peer-UID auth: a same-process
// (same-UID) peer passes, and a deliberately wrong expected UID is rejected.
func TestVerifyPeerUIDLinux(t *testing.T) {
	sock := tempSock(t)
