...
**Progress (2026-06-04):** Phase 6 (corroboration severity hardening, CORR-01/02 — F1) ✅ complete & verified. Phase 7 (PLCY-05/SPATH-01..04 — F2) ✅ complete & verified — `EvaluatePath`/`DefaultSensitivePaths` is now wired live into `runCheck`: credential reads (`~/.aws/credentials`, `~/.ssh/id_rsa`, `.env`, MCP configs) and `cat`/`type %USERPROFILE%`/`Get-Content` shell targets block fail-closed, traversal/tilde/Windows-env-var bypasses canonicalized away, `.env.example/.test/.schema` allowlisted; overlay can escalate but never downgrade a path block. Phase 8 (NUDGE-01..08 / BTEST-01..03 — F3 + the behavioral test suite) ✅ complete & verified — F3 closed (pnpm/bun/yarn installs now parsed via the single pure `internal/pkgparse` → ecosystem "npm" and catalog-matched); `internal/nudge` ships a pure `Evaluate` (soft-advise default / hard-rewrite + `requireHardened` block opt-in) wired live into check (fresh `DetectStateFn`), gateway (60s cache), and shim, with the `beekeeper nudge status|check|audit` CLI; the behavioral test suite (table-driven §10 1-10/14-17, `RunCheck` integration, fuzz targets, and the `//go:build e2e` live-binary battery over SPATH+CORR+NUDGE) is green and gates the release. **All three v1.2.0 phases complete — milestone "Runtime Behavioral Hardening" ready for close.**
...
---
*Last updated: 2026-06-04 — v1.2.0 ALL THREE PHASES COMPLETE & verified: Phase 6 (CORR, F1), Phase 7 (SPATH, F2), Phase 8 (NUDGE + BTEST, F3 + behavioral test suite / live-binary E2E release gate). Milestone "Runtime Behavioral Hardening" ready for close (`/gsd-audit-milestone` → `/gsd-complete-milestone`). v1.1.0 Pollen still parked at its maintainer release checkpoint (not closed).*
