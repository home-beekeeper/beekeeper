...
- **The repo has never been pushed.** `gh repo create bantuson/beekeeper` is part of the deferred v1.1.0 release runbook. So the CI matrix's live GitHub run is confirmed at first push — Phase 21 statically authors + locally build-verifies the YAML; it does not get a green GitHub badge until the push happens (note this honestly in the verification, like Phase 19's D-03).
