...
> ⏸ **PARKED SNAPSHOT — milestone NOT closed.** Preserved here on 2026-06-03 when the active `.planning/REQUIREMENTS.md` was switched to v1.2.0. v1.1.0 is paused at its 05-05 maintainer release checkpoint (signed tags `pollen.2/.3/.4/.5` pending — see STATE.md "Deferred Items", `HANDOFF.json`, and `docs/release-runbook.md`). This is a snapshot, not a completion archive; do not treat v1.1.0 as done. Most requirements below are code-complete; the signed releases (FORK-03 follow-through) and final tracking remain.
...
Requirements for milestone v1.1.0. Each maps to exactly one roadmap phase. Spans two repos: the new `github.com/home-beekeeper/pollen` module and beekeeper's `internal/inventory/` integration.

### Fork Setup & Hygiene (FORK)

- [x] **FORK-01**: Pollen exists as a separate Go module `github.com/home-beekeeper/pollen`, forked from `perplexityai/bumblebee` at a pinned commit (v0.1.1 tag), with `cmd/bumblebee/` renamed to `cmd/pollen/`; the `pollen` CLI binary builds on ubuntu/macos/windows
